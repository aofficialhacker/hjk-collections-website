...
const HJKTracking = {
    ...
    async trackOrder(e) {
        ...
        resultDiv.innerHTML = '<div class="text-center py-4"><div class="spinner"></div></div>';

        try {
            const res = await HJKAPI.tracking.search(orderNumber);
            if (!res.success) {
            ...
            resultDiv.innerHTML = `
                <div class="text-center py-4">
                    ...
                    <p class="text-muted" style="font-size:0.9rem">No order found with number <strong>${orderNumber}</strong>. Please check and try again.</p>
                </div>`;
        }
    }
};
