...
const AdminSettings = {
    ...
    saveGeneral(e) {
        ...
        settings.tagline = document.getElementById('siteTagline').value.trim();
        settings.email = document.getElementById('siteEmail').value.trim();
        settings.phone = document.getElementById('sitePhone').value.trim();
        settings.whatsapp = document.getElementById('siteWhatsapp').value.trim();
        settings.address = document.getElementById('siteAddress').value.trim();
        ...
        settings.mapUrl = document.getElementById('mapUrl').value.trim();
        HJKUtils.store.set('hjk_settings', settings);
        AdminComponents.showToast('Settings saved!', 'success');
    },

    initBanners() {
        if (!AdminComponents.getAdminPageShell('banners', 'Banners')) return;
        const banners = HJKUtils.store.get('hjk_banners') || [];
        const content = document.getElementById('adminContent');
                        ...
                        <div class="admin-card">
                            <div style="position:relative">
                                ...
                                <div style="position:absolute;top:10px;right:10px;display:flex;gap:6px">
                                    <button class="table-action-btn edit" style="background:#fff" onclick="AdminSettings.showBannerForm('${i}')"><i class="fa-solid fa-pen"></i></button>
                                    <button class="table-action-btn delete" style="background:#fff" onclick="AdminSettings.deleteBanner(${i})"><i class="fa-solid fa-trash"></i></button>
                                </div>
                            </div>
                            <div style="padding:16px">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <label class="toggle-switch" style="transform:scale(0.8)">
                                        <input type="checkbox" ${b.isActive ? 'checked' : ''} onchange="AdminSettings.toggleBanner(${i},this.checked)">
    ...
    showBannerForm(index) {
        const banners = HJKUtils.store.get('hjk_banners') || [];
        const banner = index !== undefined ? banners[index] : null;
        ...
        document.getElementById('bannerFormModal').innerHTML = `
            <div style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:2000;display:flex;align-items:center;justify-content:center">
                <div style="background:#fff;border-radius:var(--radius-lg);padding:30px;max-width:500px;width:90%">
                    ...
                    <form class="admin-form" onsubmit="AdminSettings.saveBanner(event,${index !== undefined ? index : -1})">
    ...
    saveBanner(e, index) {
        ...
        if (index >= 0 && banners[index]) { Object.assign(banners[index], data); }
        else { banners.push(data); }

        HJKUtils.store.set('hjk_banners', banners);
        AdminComponents.showToast('Banner saved!', 'success');
        this.initBanners();
    },

    toggleBanner(index, isActive) {
        const banners = HJKUtils.store.get('hjk_banners') || [];
        if (banners[index]) { banners[index].isActive = isActive; HJKUtils.store.set('hjk_banners', banners); }
    },

    deleteBanner(index) {
        AdminComponents.showConfirm('Delete Banner', 'Are you sure?', () => {
            const banners = HJKUtils.store.get('hjk_banners') || [];
